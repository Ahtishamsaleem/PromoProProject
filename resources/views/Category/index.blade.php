...
<main class="main-content">
	<div class="container-fluid topbarCustomPadding">
		...
		<div class="page-title d-flex mb-3 justify-content-bentween">
			<h3 class="title fw-bold m-0">Category</h3>
						...
						<ul class="nav nav-tabs nav-tabs-default mb-3 border-bottom" id="productTabsContainer"
							...
							<li class="nav-item" role="presentation">
								<button class="nav-link active" id="distributors-tab" data-bs-toggle="tab"
									...
									aria-controls="distributors-tab-pane" aria-selected="true">Category
								</button>
							</li>
						...
						<div class="tab-pane fade show active" id="distributors-tab-pane" role="tabpanel"
							...
							<div class="grid-container">
								<div class="add-new-sku-buttons px-3 mb-3">
									<a type="button" href="{{route('categories.create')}}" class="btn btn-sm px-3 btn-primary me-2" id="add-new-distributor">Add New Category</a>
								</div>
								<!-- GRID TABLE -->
								<table id="paymentcollection-grid" class="table table-striped borderless tableNowrap m-0"
									...
									<tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $value['category_company_code'] }}</td>
                                            <td>{{ $value['status'] }}</td>
                                            <td>
                                                <a href="{{ route('categories.show', $value->id) }}" class="btn btn-sm border-0 p-0" type="button"><i class="icon-outline-eye font-size-16 text-primary"></i></a>
                                                <a href="{{ route('categories.edit', $value->id) }}" class="btn btn-sm border-0 p-0" type="button"> <i class="icon-outline-edit-2 text-primary font-size-14 px-1"></i></a>
                                            </td>
                                        </tr>
										@endforeach
									</tbody>
